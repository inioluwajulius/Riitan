...
export function AtelierSection() {
  ...
  return (
    <section id="atelier" className="py-28 px-6 md:px-12 bg-[#F5EFE6] text-[#0D2218] relative border-b border-[#0D2218]/15">
      <div className="max-w-7xl mx-auto space-y-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {ATELIER_PILLARS.map((pillar) => (
            <div
              key={pillar.label}
              className="p-8 rounded-3xl bg-[#FAF7F2] border border-[#0D2218]/10 hover:border-[#8E6D30] transition-all duration-300 hover:shadow-xl space-y-4"
            >
              <div className="text-3xl sm:text-4xl font-serif font-light text-[#8E6D30]">
                {pillar.stat}
              </div>
              <h3 className="text-lg font-serif text-[#0D2218] font-normal">
                {pillar.label}
              </h3>
        ...
        <div className="p-8 sm:p-12 rounded-3xl bg-[#0D2218] text-[#FAF7F2] border border-[#C9A86A]/30 shadow-2xl relative overflow-hidden">
          {/* Ambient Glow */}
          <div className="absolute top-0 right-0 w-96 h-96 bg-[#C9A86A]/10 rounded-full blur-[100px] pointer-events-none" />

          <div className="relative z-10 space-y-12">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#C9A86A]/20 pb-6">
              <div>
                <span className="text-[10px] font-sans tracking-[0.3em] text-[#C9A86A] uppercase">
                  THE PROCESS
                </span>
                <h3 className="text-2xl sm:text-3xl font-serif text-[#FAF7F2] mt-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {steps.map((s) => {
                ...
                return (
                  <div key={s.num} className="space-y-3 relative">
